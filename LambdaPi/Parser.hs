...
parseITerm_ 0 e =
      do
        ...
        (vn:vns,t:ts) <- parseBindings_ True e
        reserved lambdaPi "."
        t' <- parseCTerm_ 0 (vn:vns)
        return (foldl (\ p (vn,t) -> Pi_ vn t (Inf_ p)) (Pi_ vn t t') (zip vns ts))
  <|>
  try
     (do
        t <- parseITerm_ 1 e
...
parseITerm_ 3 e =
  ...
  <|> do
        ...
        case findIndex (== x) e of
          Just n  -> return (Bound_ n "inventedboundname")
...
parseLam_ e =
      do reservedOp lambdaPi "\\"
         ...
         --  reserved lambdaPi "."
         --CHANGED return (iterate Lam_ t !! length xs)
         return (foldr ($) t (map Lam_ xs)) 
